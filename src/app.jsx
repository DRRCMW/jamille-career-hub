...
const RESUME = {
  ...
  education:[{ degree:"Bachelor of Science, Business Administration", school:"[CONFIRM UNIVERSITY — see note in Resume tab]", location:"Philadelphia, PA", needsConfirm:true }],
};
...
/* ─── STORAGE HOOK (localStorage + Claude storage fallback) ─── */
function useStorage(key, fallback) {
    ...
    (async () => {
      try {
        ...
        if (!local && window.storage) {
          ...
          if (r?.value) { const p = JSON.parse(r.value); setVal(p); localStorage.setItem(key, r.value); }
        }
      } catch {}
    })();
  }, [key]);
...
const Btn = ({ children, onClick, variant="gold", disabled, full, icon, style={} }) => {
  ...
  return (
    <button onClick={disabled?undefined:onClick} style={{
      ...
      gap:6, width:full?"100%":undefined, transition:"all 0.18s",
      ...style }}>
      {icon && <span>{icon}</span>}{children}
    </button>
  );
